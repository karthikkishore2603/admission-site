...
def submit_formone():
    try:
        ...
        if crud.check_formone(form_data['phone']):
...
#FORMTWO
@app.route('/api/formtwo', methods=['POST'])
def submit_formtwo():
    try:
        form_data = request.json  # Assuming the data is sent as JSON
        # Process the form data here
        ...
        print("Checking if formtwo data already exists with phone number:", form_data['phone'])

        # if crud.check_formtwo(form_data['phone']):
        #     print("Formtwo data already exists with phone number:", form_data['phone'])
        # else:
        #     crud.store_formtwo(form_data)
        #     print("Formtwo data stored successfully")
